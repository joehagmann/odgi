...
namespace odgi
{
    namespace algorithms
    {
...
#ifdef debug_bin_path_info
            std::cerr << "AFTER_UPDATE_PAIR: "
                      << "<" << std::get<first>(p) << "," << std::get<second>(p) << ">"
                      << std::endl;
#endif
        }
        ...
        int get_count_of_link_in_bin(const std::vector<link_info_t> &links, const link_info_t &link)
        {
            ...
            {
                if (l.from == link.from && l.to == link.to)
                    count++;
            }
            return (count);
        }
        ...
        {
            std::cout << "  link from " << l.from << " to " << l.to << ", " << l.times << "x, " << l.nr_passes << " passes" << std::endl;
        }

        bool is_not_adjacent_link(link_info_t link, uint64_t adj_link)
        {
            if (link.from == adj_link || link.to == adj_link)
                return (false);
            else
                return (true);
        }
                                          ...
                                          uint64_t bin_width)
        {
            ...
            std::map<std::string, GeneAnnotation> annotations; // TODO: we could think of concat all gff3 files and read in only one GeneAnnotation instance
            for (auto p : ref_paths)
            {
                std::cout << p.first << " " << p.second << std::endl;
                //handle_annotation(graph.get_path_name(path), anno_file, ref2bin);
                // p.first = reference ID, p.second = gff3 file
                std::string annofile = p.second;
                GeneAnnotation anno;
                anno.parse_gff3_annotation(annofile, false); // TODO: do that once before all the loops
                std::cout << "parsing ok" << std::endl;
                annotations.insert(std::make_pair(p.first, anno));
            ...
            graph.for_each_path_handle([&](const path_handle_t &path) {
                ...
                std::cout << "iterating path: " << path_name << std::endl;
                std::map<uint64_t, bin_info_t> bins;

                std::map<uint64_t, uint64_t> ref2bin;
                std::unordered_map<uint64_t, std::set<std::string>> bin2genes;
                //std::vector<Gene> genes;
                std::vector<Gene>::iterator gene_iter;
                bool ref_path = false;
                if (ref_paths.find(path_name) != ref_paths.end())
                {
                    ref_path = true;
                    // genes = annotations[path_name].gene_dict[path_name];
                    // gene_iter = genes.begin();
                    if (!annotations[path_name].gene_dict.empty())
                    {
                        std::cout << annotations[path_name].gene_dict[path_name][0] << std::endl;
                        for (auto &g : annotations[path_name].gene_dict[path_name])
                        {
                            std::cout << g << std::endl;
                        }
                    }
                    //                         gene_iter = annotations[path_name].gene_dict[path_name].begin();
                    //                     for (; gene_iter != annotations[path_name].gene_dict[path_name].end(); ++gene_iter)
                    //                     {
                    // std::cout << *gene_iter << std::endl;
                    //                     }
                    gene_iter = annotations[path_name].gene_dict[path_name].begin();
                }
                std::cout << "path: " << path_name << " reference? " << ref_path << std::endl;
                std::cout << "  ref_paths: ";
                for (auto p : ref_paths)
                {
                    std::cout << p.first << ",";
                ...
                graph.for_each_step_in_path(path, [&](const step_handle_t &occ) {
                    ...
                    for (uint64_t k = 0; k < hl; ++k)
                    {
                        ...
                        if (ref_path)
                        {
                            // is there a gene overlapping?
                            //                                 if (gene_iter->st <= path_pos && path_pos <= gene_iter->en) {
                            //                                     // store mapping refcoord -> bin
                            //                                     ref2bin[path_pos] = curr_bin;
                            //                                     bin2genes[curr_bin].insert(gene_iter->gene_id);
                            // std::cout << "bin " << curr_bin << " path_pos: " << path_pos << " gene: " << gene_iter->gene_id << std::endl;
                            //                                 }
                            // are there genes overlapping ref_pos?
                            std::vector<Gene>::iterator genes_to_check = gene_iter;
                            while (genes_to_check->st <= path_pos && path_pos <= genes_to_check->en)
                            {
                                ref2bin[path_pos] = curr_bin;
                                std::string geneID = genes_to_check->gene_id;
                                std::cout << "gene to check: " << geneID << std::endl;
                                if (bins[curr_bin].genes.find(geneID) == bins[curr_bin].genes.end())
                                {
                                    gene_info_t gene;
                                    bins[curr_bin].genes.insert(std::make_pair(geneID, gene));
                                }
                                bins[curr_bin].genes[geneID].exons.insert(genes_to_check->in_which_exon(Interval(path_pos, path_pos)));
                                if (genes_to_check->snd == -1)
                                {
                                    bins[curr_bin].genes[geneID].strand = false;
                                }
                                else if (genes_to_check->snd == 1)
                                {
                                    bins[curr_bin].genes[geneID].strand = true;
                                }
                                else
                                {
                                    std::cerr << "[odgi bin] warning: strand of gene " << geneID << " on " << path_name << " not known!\n";
                                }

                                std::cout << "bin " << curr_bin << " path_pos: " << path_pos << " gene: " << genes_to_check->gene_id << std::endl;
                                std::cout << "  in exon? " << genes_to_check->in_exon(Interval(path_pos, path_pos)) << std::endl;
                                std::cout << "  in which exon? " << genes_to_check->in_which_exon(Interval(path_pos, path_pos)) << std::endl;

                                genes_to_check++;
                            }
                            while (gene_iter != annotations[path_name].gene_dict[path_name].end() && path_pos > gene_iter->en)
                            {
                                gene_iter++;
                            }
                        }
                ...
                while (it != bins.end())
                {
                    ...
                    if (which_links == 0)
                        links = bins[bin_id].links_to;
                    else
                        links = bins[bin_id].links_from;
            ...
            cumsum_links[0] = 0;
            for (uint64_t bin = 1; bin <= num_bins; ++bin)
            { // bin IDs start with 1
